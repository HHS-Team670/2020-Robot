...
import frc.team670.paths.twentytwentyone.WideLeft5Diagonal;
 ...
 * wide angle means robot enters switch from different quadrant than it exists from
 * for 2021 field
 * @author elisevbp
 */
public class WideShootThen5Diagonal extends SequentialCommandGroup implements MustangCommand {

    private Map<MustangSubsystemBase, HealthState> healthReqs;
    // private DriveBase driveBase;
    private Path trajectory;
    
    public WideShootThen5Diagonal(StartPosition startPosition, DriveBase driveBase, Intake intake, Conveyor conveyor, 
    Indexer indexer, Turret turret, Shooter shooter) {
        ...
        if (startPosition == StartPosition.LEFT) {
            trajectory = new WideLeft5Diagonal(driveBase);
            turretAng = RobotConstants.leftTurretAng;
        }
        if (startPosition == StartPosition.CENTER)
            trajectory = new Center5Diagonal(driveBase); //TODO: CHANGE THIS TO WIDE EQUIVALENT!!
