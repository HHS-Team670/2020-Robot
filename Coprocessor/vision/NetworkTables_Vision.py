...
VISION_VALUES_BASE_KEY = "vision-values-"

VISION_MIN_HSV_BASE_KEY = "vision-min-"
VISION_MAX_HSV_BASE_KEY = "vision-max-"
VISION_HSV_SUB_KEY = ["h", "s", "v"]

VISION_SHAPE_POINT = "vision-shapepoint-"

VISION_SHAPE_POINTS_KEY = "vision-shapepoints"
...
lower_green = np.array([table.getNumber(VISION_MIN_HSV_BASE_KEY + "h", 70),
                table.getNumber(VISION_MIN_HSV_BASE_KEY + "s", 235),
                table.getNumber(VISION_MIN_HSV_BASE_KEY + "v", 255)])

upper_green = np.array([table.getNumber(VISION_MAX_HSV_BASE_KEY + "h", 95),
        table.getNumber(VISION_MAX_HSV_BASE_KEY + "s", 255),
        table.getNumber(VISION_MAX_HSV_BASE_KEY + "v", 255)])

VISION_POINTS = table.getStringArray(VISION_SHAPE_POINTS_KEY, [])
print("VISION_POINTS (initial): \n", VISION_POINTS)
model_points = []
for index in range(len(VISION_POINTS)):
        model_points.append(np.array([
                table.getNumber(VISION_POINTS[index] + "-X", 0), 
                table.getNumber(VISION_POINTS[index] + "-Y", 0), 
                table.getNumber(VISION_POINTS[index] + "-Z", 0)]))
...
FRONT_CAPTURE_SOURCE = 0          # Video source number for front camera
...
print("initial.png created")

def updateHSV(table, key, value, isNew):
        global lower_green
        global upper_green
        lower_green = np.array([table.getNumber(VISION_MIN_HSV_BASE_KEY + "h", 70),
                table.getNumber(VISION_MIN_HSV_BASE_KEY + "s", 235),
...
def updatePoints(table, key, value, isNew):
        VISION_POINTS = table.getStringArray(VISION_SHAPE_POINTS_KEY, [])
        # model_points = np.array([table.getNumber(VISION_POINTS[i], 0) for i in range(len(VISION_POINTS))])
        model_points = []
        for index in range(len(VISION_POINTS)):
                model_points.append(np.array([
                        table.getNumber(VISION_POINTS[index] + "-X", 0), 
...
def push_network_table(table, return_list):
        ...
        '''
        table.putNumberArray(VISION_VALUES_BASE_KEY, return_list)
        NetworkTables.flush()
...
def test_candidate_contour(candidates):
        ...
        max_iterations = 5 # change this number?
        x = 0
        for c in candidates:
                # print(x)
                if x > max_iterations:
                ...
                convex = cv2.imread("test.png")
                cv2.drawContours(convex, [box_points], -1, (0,0,255), 1)
                cv2.imwrite("convex_hull.png", convex)
                print("boxpoints: ", box_points)
                points = []
...
def checkEnabled(table, key, value, isNew):
        ...
        hopefully_hexagon = test_candidate_contour(contour_list) # may not work if has to be 6 points in box_points1
        # print(hopefully_hexagon) #uncomment this stuff out, likely will be 6 points
        ...
        if len(hopefully_hexagon) > 0:
                ...
                if len(box_points1) == 4:
                        ...
                        if success:
                                distance, angle1, angle2 = compute_output_values(rotation_vector, translation_vector) #distance in m
                                print("Calc: " + str(distance / 25.4)) #inches
        ...
        table.putBoolean(enabled_key, False)
        # print(str((datetime.datetime.now() - timer).microseconds / 1000) + "ms" )
...
table.addEntryListener(checkEnabled, key=enabled_key)

for i in range(len((VISION_HSV_SUB_KEY))):
        table.addEntryListener(updateHSV, key=VISION_MAX_HSV_BASE_KEY + VISION_HSV_SUB_KEY[i])

# for i in range(len((VISION_SHAPE_POINTS_KEY))):
table.addEntryListener(updatePoints, key=VISION_SHAPE_POINTS_KEY)
table.addEntryListener(updatePoints, key=VISION_SHAPE_POINT + "1-X")
print(VISION_SHAPE_POINT + "1-X")
        # table.addEntryListener(updatePoints, key=VISION_SHAPE_POINTS_KEY[i] + "-Y")
        # table.addEntryListener(updatePoints, key=VISION_SHAPE_POINTS_KEY[i] + "-Z")
